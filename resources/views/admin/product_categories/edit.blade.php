...
<div class="u-page-header">
    <div class="u-header-info">
        <h1 class="u-header-title">تعديل تصنيف منتج</h1>
        <p class="u-header-sub">تعديل بيانات التصنيف "{{ $productCategory->name }}"</p>
    </div>
...
<div class="u-table-wrap" style="max-width: 600px;">
    <form action="{{ route('admin.product_categories.update', $productCategory) }}" method="POST" style="padding: 24px;">
        @csrf
        @method('PUT')
        ...
        <div style="margin-bottom: 20px;">
            ...
            <input type="text" name="icon" value="{{ old('icon', $productCategory->icon) }}"
                   style="width: 100%; padding: 10px 14px; border: 1px solid #d1d5db; border-radius: 10px; font-size: 1rem; outline: none;"
